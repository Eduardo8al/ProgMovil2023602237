...
class DBHelper(context: Context) :
    ...
    fun getProductoPorId(id: Int): Producto? {
        val db = readableDatabase
    ...
    fun actualizarProducto(id: Int, nombre: String, descripcion: String?, precio: Double, imagenBase64: String?) {
        val db = writableDatabase
        val values = ContentValues().apply {
            put("nombre", nombre)
            put("descripcion", descripcion)
        ...
        db.update("producto", values, "id_producto = ?", arrayOf(id.toString()))
        db.close()
    }
